...
@section('content')
    <div class="bg-white p-6 rounded-lg shadow-md w-full max-w-md mx-auto">
        ...
        <div class="font-bold mb-3 text-xl border-b-1 inline-block">{{$action}} Kategori</div>
        ...
        @isset($items)
            <form method="POST" action="{{ route('dashboard.categori.update',$items->id) }}">
            @method('PUT')
        @else
            <form method="POST" action="{{ route('dashboard.categori.store') }}">
        @endisset
            @csrf
