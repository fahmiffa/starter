...
@section('content')
    <div class="text-gray-800 text-lg bg-gray-50 rounded-2xl shadow-md mx-10" x-data="Crudunit()"
        x-init="fetchData('/dashboard/unit-json')">
        <div class="grid grid-cols-1 md:grid-cols-2 w-min-screen">
            <div class="p-6">
                <form method="POST" @submit.prevent="formHandler('/dashboard/unit')">
                    @csrf
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        <label class="block text-gray-700 text-sm font-semibold mb-2">PCS</label>
                        <input type="number" min="1" name="pcs" x-model="form.pcs"
                            class="w-full p-2 rounded border border-gray-300 focus:outline-none focus:ring focus:border-blue-300"
                            :class="errors.pcs ? 'border-red-500' : ''">
                        <p class="text-red-500 text-sm mt-1" x-text="errors.pcs?.[0]"></p>
                    </div>
                    ...
                    <table class="min-w-full bg-white border border-gray-200 text-sm">
                        <thead>
                            <tr class="bg-gray-100 text-left">
                                ...
                                <th class="cursor-pointer px-4 py-2">PCS</th>
                        ...
                        <tbody>
                            <template x-for="(row, index) in paginatedData()" :key="row.id">
                                <tr class="border-t border-gray-400">
                                    ...
                                    <td class="px-4 py-2" x-text="row.pcs"></td>
                                    <td class="px-4 py-2 flex items-center gap-1">
                                        ...
                                        <button @click="deleteItem('/dashboard/unit',row.id)"
